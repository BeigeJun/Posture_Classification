...
while cap.isOpened():
    ...
    for idx in high_scores_idx:
        ...
        black_background = draw_keypoints_and_connections(black_background, keypoints)
        ...
        if check_count < 2:
            ...
            cv2.rectangle(black_background, (x1, y1), (x2, y2), box_color, 2)

            (label_width, label_height), baseline = cv2.getTextSize(OutPut_Label, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
            cv2.rectangle(black_background, (x1, y1 - label_height - baseline), (x1 + label_width, y1), box_color, cv2.FILLED)
            cv2.putText(black_background, OutPut_Label, (x1, y1 - baseline), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

    cv2.imshow('frame', black_background)
