...
+ 09.3D_Pos 
  + 초기에 시도했던 방법입니다. 카메라 하나로는 사람의 방향을 잘 판단할 수 없다는 이유로 두개의 카메라를 이용하여 3차원 스켈레톤을 제작했습니다. 하지만 카메라가 많아지면 그만큼 복잡해지고(높은곳에서 낮은곳을 여러 카메라가 촬영할 때 그 각도로 생긴 차이를 어떻게 한 사람으로 재구성할 방법을 못찾음) 한 사람을 감지하기 위해 여러 카메라가 쓰이는건 적절하지 않다고 생각했기 때문에 폐기 하였습니다. 프로그램은 카메라가 사람의 정면, 측면을 촬영하면 3차원 애니매이션으로 그래프가 그려집니다.
